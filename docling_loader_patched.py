...
class DoclingReader(BaseReader):
    ...
    def converter_(self):
        ...
        # Check CUDA availability (can be forced off with DOCLING_FORCE_CPU=1)
        force_cpu = os.environ.get("DOCLING_FORCE_CPU", "").lower() in ("1", "true", "yes")
        cuda_available = torch.cuda.is_available() and not force_cpu
        print(f"[Docling] CUDA available: {cuda_available} (force_cpu={force_cpu})")

        # Configure OCR with GPU
        ...
        table_options = TableStructureOptions(
            ...
            mode=TableFormerMode.ACCURATE,  # Better quality
        )
        ...
        # Try newer AcceleratorOptions if available (Docling 2.6+)
        try:
            from docling.datamodel.pipeline_options import AcceleratorOptions, AcceleratorDevice
            if cuda_available:
                accelerator = AcceleratorOptions(device=AcceleratorDevice.CUDA)
                print(f"[Docling] Using AcceleratorOptions with CUDA")
        ...
        # Fallback for Docling 2.5.x
        print(f"[Docling] Using PdfPipelineOptions (OCR GPU: {cuda_available})")
        return DocumentConverter(
            format_options={
                InputFormat.PDF: PdfFormatOption(pipeline_options=pipeline_options)
            }
